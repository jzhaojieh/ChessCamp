  ...
  def index
    if logged_in? && current_user.role?(:parent)
      unless Family.where(user_id: current_user.id).empty?
        ...
        @upcoming_camps = Camp.active.upcoming.where.not(id: Camp.upcoming.full).where(curriculum_id: Family.where(user_id: current_user.id).first.students.map{|a| a.rating}.map {|r| Curriculum.for_rating(r)}.map{|d| d.ids}.flatten).chronological.paginate(:page => params[:page]).per_page(5)
      ...
      @r_camps = Camp.where.not(id:  CampInstructor.all.map{|a| a.camp.id}).active.chronological.paginate(:page => params[:page]).per_page(5)
